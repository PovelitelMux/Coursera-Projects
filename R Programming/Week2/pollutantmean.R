...
pollutantmean <- function(directory, pollutant = "sulfate", id = 1:332) {
    ...
    # Concatenate vectors directory and all_files after converting to character 
    file_paths <- paste(directory, all_files, sep="")
    # циклічно перебираємо всі необхідні файли в папці specdata
    for(i in id) {
        # читаємо необхідний файл
        current_file <- read.csv(file_paths[i], header=T, sep=",")
        head(current_file)
        pollutant
        # видаляємо втрачені дані
